...
class ControlBarDropdownButton extends React.Component {
  ...
  @autobind @action onToggleTooltip(isOpen) {
    this.isTooltipOpen =
      !this.props.$$udControlBarDropdown.props.isOpen && isOpen;
  }
  ...
  @autobind onKeyDown(event) {
    ...
    if (isUp || isDown) {
      ...
      !$$udControlBarDropdown.props.isOpen &&
        $$udControlBarDropdown.props.onToggle(true);
    ...
    return (
      ...
      >
        ...
        {props.role === "menuitemcheckbox" && (
          <span styleName="checkbox-slider" />
        )}
      </BlockList.Item>
    );
  }
...
export default class ControlBarDropdown extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.triggerId =
      ...
      getUniqueId("control-bar-dropdown-trigger");
  ...
  @autobind findMenuItemNodes() {
    ...
    const items = menu
      ? menu.querySelectorAll("button,a[href],[tabindex]")
      : [];
  ...
  render() {
    const { children, darkMode, menuMaxHeight, menuOffset, ...props } =
      this.props;
    return (
      <Provider
