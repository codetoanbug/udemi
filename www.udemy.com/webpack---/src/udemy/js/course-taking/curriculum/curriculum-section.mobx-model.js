...
export default class CurriculumSection extends CurriculumItem {
  ...
  get apiDataMap() {
    return {
      ...
      chapterGtAssignmentIds: {
        ...
        map: (gtAssignments) => {
          return gtAssignments.map(
            (gtAssignment) => gtAssignment.gt_instance.id
          );
        },
        defaultValue: [],
      },
  ...
  get numCompletedItems() {
    return this.items.filter((item) => item.isCompleted && item.isPublished)
      .length;
  }
    ...
    return !this.items
      .filter(
        (item) =>
          item.type === "lecture" &&
          item.asset &&
          (item.asset.type === ASSET_TYPE.VIDEO ||
            item.asset.type === ASSET_TYPE.AUDIO)
      )
      .some((item) => {
        return !item.isCompleted;
      });
