...
export const InstructorStats = ({ instructor }) => {
  ...
  const numRatingText =
    instructor.total_num_reviews === 0
      ? "--"
      : formatNumber(instructor.total_num_reviews);
  const numStudentText =
    instructor.total_num_students === 0
      ? "--"
      : formatNumber(instructor.total_num_students);
  const numCourseText =
  ...
  return (
    <BlockList size="small" padding="tight">
      ...
      <BlockList.Item
        data-purpose="stat"
        key={1}
        icon={<CertificateIcon label={false} />}
      >
        {ninterpolate(
          "%(count)s Review",
          "%(count)s Reviews",
...
export const InstructorTitle = ({ instructor }) => {
  return (
    <div>
      <div
        styleName="instructor__title"
        className="ud-heading-lg ud-link-underline"
      >
        <a href={instructor.absolute_url || instructor.url}>
          {instructor.title}
        </a>
      </div>
