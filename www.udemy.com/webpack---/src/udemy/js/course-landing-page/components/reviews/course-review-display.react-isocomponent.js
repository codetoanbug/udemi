...
export default class CourseReviewDisplay extends Component {
    ...
    return (
      <div data-purpose="course-reviews">
        ...
        {!this.store.isReviewSectionVisible ? (
          <NoReviewsMessage
            message={reviewSectionUnavailableMessage(this.store)}
          />
        ) : (
  ...
  trackRatingsSummaryImpression() {
    const { courseTrackingId, reviewableObjectId, reviewableObjectType } =
      this.props;
    if (courseTrackingId) {
  ...
  renderRatingsAndReviews() {
    return (
      <div data-purpose="ratings-and-reviews">
        ...
        <TopReviewAttributes
          topReviewAttributes={this.store.topReviewAttributes}
        />
...
export const SearchedTerm = observer(({ store }) => {
  ...
  if (store.reviews.length) {
    ...
    return (
      <>
        {isRatingSelected && (
          <span className="ud-sr-only">{ratingApplied}</span>
        )}
        <div
