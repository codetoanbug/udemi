...
export function UdemyForBusinessTabContent({ primaryLink, buttonText }) {
  ...
  const hasDarkBackground = useMatchMedia(
    "(min-width: 701px) and (max-width: 1079px)"
  );
      ...
      <BlockList size="small" iconAlignment="left">
        ...
        <BlockList.Item icon={<TickIcon label={false} />}>
          {interpolate(
            ...
            {
              courseCount:
                getOrgNumericSiteStat("num_courses").toLocaleString(),
            },
            true
          )}
        </BlockList.Item>
