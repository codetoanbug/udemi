        ...
        <BlockList iconAlignment="left" size="small">
          {items.map((test) => {
            ...
            return (
              <BlockList.Item
                key={test.id}
                icon={<PracticeTestIcon label={false} />}
              >
...
export class Section extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.expanded =
      this.props.areAllSectionsExpanded || this.props.arrayIndex === 0;
  }

  componentDidUpdate(prevProps) {
    if (
      prevProps.areAllSectionsExpanded !== this.props.areAllSectionsExpanded
    ) {
  ...
  renderTitle({ children, className, ...props }) {
    return (
      <h3
        {...props}
        className={classNames(className, styles["section-title-container"])}
      >
        {children}
      </h3>
    );
...
export class Lecture extends React.Component {
  ...
  render() {
    ...
    const hasLectureLandingLink =
      !udConfig.brand.has_organization && lecture.landing_page_url;
    const ToggleDescriptionIcon = this.showDescription
      ? CollapseIcon
      : ExpandIcon;
    ...
    if (hasLectureLandingLink) {
      ...
      LectureTitle = Button;
      Object.assign(lectureTitleProps, {
        udStyle: "link",
        typography: "ud-text-sm",
      });

      const isLecturePopup = previewLink.includes(
        `/new-lecture/${lecture.id}/popup/`
      );
      coursePreviewModal = (
        <CoursePreviewModal
      ...
      <>
        ...
        >
          ...
          {previewLink && showSectionContentsText && (
            <span styleName="hidden-on-mobile preview-text">
              {gettext("Preview")}
            </span>
          )}
