...
function shoppingClient() {
  ...
  const state = {
    ...
    lists: {
      ...
      saved_for_later: shoppingList(
        "saved_for_later",
        store.lists.saved_for_later
      ),
  ...
  try {
    const items =
      JSON.parse(window.sessionStorage.getItem("checkoutItems")) || [];
  ...
  class Client {
    ...
    validateDiscounts() {
      ...
      Object.entries(notApplicableCodes).forEach(([type, codes]) => {
        const string =
          config.errors.discount[type][
            codes.length > 1 ? "plural" : "singular"
          ];
    ...
    get isCheckoutAvailable() {
      return Boolean(
        state.lists.checkout && !state.lists.checkout.isEmpty && udMe.id
      );
    }
  ...
  function performTerminalAction(data, buyables) {
    if (getIsMobileApp()) {
      ...
      webviewBridge.sendMessage(
        new OnEnrolledMessage(buyables, data.purchasePrice)
      );
    } else {
