...
export default function shoppingStorage() {
  ...
  storage.fetch = () => {
    if (
      !getConfigData().features.shopping_cart ||
      (!state.userId && !state.etag)
    ) {
      runInAction(() => {
  ...
  storage.removeItem = (listName, item) => {
    ...
    function resolveConflicts(req, results) {
      const serverBuyables = (results[listName] || []).map(
        (item) => item.buyable
      );
  ...
  const updateStateOnSuccess = action((response) => {
    ...
    state.credit.amount = response.data.user.id
  ...
  function createShoppingError(httpError) {
    return Promise.reject({
      get canBeResolved() {
        return (
          httpError.response.status === 412 || httpError.response.status === 409
        );
      },
  ...
  function makeDeferredRequest(request, conflictResolver) {
    function commandResolver(command, requestError) {
      ...
      const conflictsResolved = conflictResolver(
        request,
        requestError.refetchedData
      );
