...
function groupVideoJSErrors(event) {
  ...
  if (
    frames.length > 0 &&
    frames[frames.length - 1].filename.includes("videojs")
  ) {
...
class Sentry {
  ...
  _initializeSentry(Sentry, options = {}) {
    ...
    const defaultOptions = {
      ...
      whitelistUrls:
        udConfig.env === "PROD" ? [/udemy\.com/, /udemy\.cn/] : [/./],
      sampleRate: udConfig.env === "PROD" ? 0.05 : 1,
      autoSessionTracking: false,
    };
    Sentry.init(
      Object.assign(this.previousSentryOptions, defaultOptions, options)
    );
    when(
      () => !udMe.isLoading,
      () => {
    ...
    Sentry.setTag(
      "page_key",
      udUserAgnosticTrackingParams.page_key ||
        `${udConfig.app_name}-without-page-key`
    );
