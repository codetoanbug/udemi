...
export class HotkeyRegistry {
  ...
  getModifiers(key, event) {
    ...
    const isModifiable = Object.values(HotkeyRegistry.MODIFIABLE_KEYS).includes(
      key
    );
  ...
  handleKeyDown(event) {
    const activeElement =
      event.target || event.toElement || document.activeElement;
    const isInteractable =
      !!activeElement && isInteractableElement(activeElement);
    ...
    handlers.forEach((handler) => {
      const shouldCall = handler.shouldCall
        ? handler.shouldCall(event)
        : !isInteractable;
