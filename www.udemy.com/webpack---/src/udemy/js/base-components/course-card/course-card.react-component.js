...
const CoursePrice = ({ course, ...props }) => {
  const { discountPrice, discountPriceString, listPrice, listPriceString } =
    getPriceInfo(course);
  return (
    <StaticPriceText
...
const CourseRatings = ({ course, numReviewsText, children }) => {
  ...
  const numReviewsTextFinal =
    numReviewsText || formatNumber(course.num_reviews);
  return (
    <div className={styles.row}>
      {!!course.rating && (
        <>
          ...
          <span
            aria-label={getNumReviewsAriaLabel(course.num_reviews, {
              ninterpolate,
            })}
...
const CourseCardBadges = ({ course, children }) => {
  ...
  const CourseBadgeComponent =
    badgeData && getCourseBadgeFromType(badgeData.badge_family);
