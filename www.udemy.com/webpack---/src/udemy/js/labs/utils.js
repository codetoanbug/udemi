...
export const getAdditionalUsageTermsForVerticalAndProvider = (
  vertical,
  provider
) => {
  if (vertical === LAB_VERTICAL.aws.key) {
    ...
    vertical === LAB_VERTICAL.data_science.key &&
    provider !== LAB_PROVIDER.udemy
  ) {
    return interpolate(
      gettext(
...
export const sendLabHeartbeatEvent = (
  uuid,
  startTimeMs,
  elapsedTimeMs,
  labCompletionMode
...
export const sendLabSelectedEvent = ({
  lab,
  uiRegion,
  trackingId,
  sourcePageId,
...
export const generateLabTakingSessionEntity = (
  labInstance,
  sessionStartTime
) => {
  if (!labInstance) {
...
export const sendLabExitedEvent = (
  lab,
  uiRegion,
  sessionStartTime,
  isLabInstancePaused
...
export const sendLabResumedEvent = (
  lab,
  uiRegion,
  sessionStartTime,
...
export const sendLabTaskCompletedEvent = (lab, labTask, taskNumber) => {
  ...
  const labTaskHasAlr =
    hasAutomatedReviewTests && LAB_MODES_WITH_ALR.includes(lab.currentMode);
  const labEntity = generateLabEntity(lab);
  const labTakingSession = generateLabTakingSessionEntity(
    lab?.myLatestInstance,
...
export const sendLabInCoursePromptActionEvent = ({ courseId, action }) => {
  Tracker.publishEvent(
    new LAB_TRACKING_EVENTS.LAB_IN_COURSE_PROMPT_ACTION({ courseId, action })
  );
};

export const sendLabCardLandingPageClickEvent = (
  labId,
  page,
  index,
  uiRegion
...
export const sendLabsResumeBannerDismissClickEvent = (lab) => {
  ...
  Tracker.publishEvent(
    new LAB_TRACKING_EVENTS.RESUME_BANNER_DISMISS_CLICK(eventData)
  );
};
...
export const sendLabsResumeBannerEndLabClickEvent = (lab) => {
  ...
  Tracker.publishEvent(
    new LAB_TRACKING_EVENTS.RESUME_BANNER_END_LAB_CLICK(eventData)
  );
};

export const sendLabTaskReviewRequestedEvent = (
  lab,
  labTask,
  taskNumber,
  ...
  const labTaskEntity = new LabTaskEntity(
    id,
    taskNumber,
    title,
  ...
  Tracker.publishEvent(
    new LAB_TRACKING_EVENTS.LAB_TASK_REVIEW_REQUESTED(eventData)
  );
};

export const sendAutomatedLabReviewEvent = (lab, action) => {
  const labHasALR = lab.tasks.some(
    (task) => task.hasAutomatedReviewTests === true
  );
  const eventData = {
    labId: lab.id.toString(),
  ...
  Tracker.publishEvent(
    new AutomatedLabReviewEvent(EVENT_LABEL[action], eventData)
  );
};

export const sendAutomatedLabReviewCheckOutputViewEvent = (
  lab,
  testId,
  outputMessageType
) => {
...
export const checkLabHighlyConsumed = (lab) => {
  ...
  if (instance && instance.totalSpend) {
    const allowedSpend =
      instance.allowedSpend || DEFAULT_ALLOWED_SPEND_FOR_INSTRUCTOR;
...
export const checkUserLabAccessLevelWithExperiment = (
  ...
  const variantValue = getVariantValueFromUdRequest(
    experimentSet,
    variantName,
    defaultValue
  );
  return (
    variantValue === expectedValue && checkUserLabAccessLevel(suitableLevels)
  );
};
...
export const toLabTaking = (url, subPath, params) => {
  return udLink.to(
    url,
    subPath ? `lab-overview/tasks/${subPath}` : "lab-overview",
    params
  );
};
...
export const getLabDiscoveryCardExperimentContent = () => {
  const title = getVariantValueFromUdRequest(
    "lab_taking",
    "Title",
...
export const sendLabLoadingNotificationEvent = (labId, labVertical, action) => {
  Tracker.publishEvent(
    new LabLoadingNotification(
      NOTIFICATION_EVENT_LABEL[action],
      labId,
      labVertical
    )
  );
...
export const getVerticalIconComponent = (lab) => {
  ...
  return LabVerticalIconComponent ? (
    <LabVerticalIconComponent label={false} />
  ) : null;
};
