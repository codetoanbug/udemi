...
export default class Lab extends EditableApiModel {
  ...
  constructor(apiData, globalOverrides = {}) {
    ...
    reaction(
      ...
      () => {
        this.enrollment &&
          this.enrollment?.setIsCompleted(this.areTasksCompleted);
      }
    );
    reaction(
  ...
  get verticalLabel() {
    if (
      (this.vertical === LAB_VERTICAL.web.key ||
        this.vertical === LAB_VERTICAL.devops.key) &&
      this.specName
    ) {
  ...
  get isPreviousResourcesPanelVisible() {
    return (
      ...
      this.previousTasks.some((task) => task.hasResources)
    );
  }
  ...
  get activeTaskReviewMessage() {
    ...
    const indexInPool =
      (this.activeTaskNumber - 1 - 2) % TASK_REVIEW_MESSAGES.pool.length;
  ...
  @computed
  get canPublish() {
    const userHasSpecialAccess = checkUserLabAccessLevel(
      FULL_AND_ADMIN_LAB_ACCESS_LEVELS
    );
    const userHasPermissions =
      this.instructorHasLab &&
      this.instructorHasLab.isOwner &&
      this.instructorHasLab.hasPermission(
        LAB_INSTRUCTOR_PERMISSIONS.EDIT_PUBLISHED
      );
    return (
      checkLabVersioningLabManageExperimentEnabled() &&
  ...
  get formErrors() {
    ...
    const errors = {};
    if (
      this.validationErrors?.min_estimated_time ||
      this.reviewError?.min_estimated_time
    ) {
      errors.min_estimated_time = gettext(
        "Please enter a number between 45 and 180"
      );
    }
    if (
      this.validationErrors?.max_estimated_time ||
      this.reviewError?.max_estimated_time
    ) {
  ...
  get hasAutomatedReviewTests() {
    return (
      this.tasks?.length > 0 &&
      this.tasks.some((task) => task.hasAutomatedReviewTests)
    );
  }
  ...
  _onPublishFailed(message) {
    const title = interpolate(
      gettext("Publish failed: %(message)s"),
      { message },
      true
    );
    toasterStore.addAlertBannerToast(
  ...
  async reorderTask(oldPosition, newPosition) {
    ...
    try {
      await udApi.post(
        `${labTasksApiUrl(this.id)}${this.tasks[oldPosition].id}/reorder/`,
        {
