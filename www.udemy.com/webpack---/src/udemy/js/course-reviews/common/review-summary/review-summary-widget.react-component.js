...
import {
  StarRating,
  StarRatingSizes,
} from "@udemy/react-merchandising-components";
...
export default class ReviewSummaryWidget extends Component {
  ...
  getAriaTextForRating(rating) {
    return interpolate(
      gettext("Rating: %(rating)s out of 5"),
      { rating },
      true
    );
  }
  ...
  renderAverageRating() {
    ...
    return (
      <div
        styleName="average-container"
        className={averageRatingClass}
        {...props}
      >
        <div
  ...
  renderRatingGauges() {
    ...
    const ratingDistributionWithPercentage = this.props.ratingDistribution.map(
      (range) => ({
        rating: range.rating,
        count: range.count,
        percentage:
    ...
    if (totalPercentage !== 100) {
      ...
      ratingDistributionWithPercentage[highestRangeIndex].percentage +=
        100 - totalPercentage;
    }
