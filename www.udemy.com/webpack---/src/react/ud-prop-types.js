  ...
  return (originalValidator) => {
    function validator(
      ...
      if (
        ...
        props[conditionalPropName] !== undefined
      ) {
        return new Error(
...
export function instanceOfComponent(...types) {
  const _propType = (required, props, propName, componentName) => {
    ...
    const instances =
      propName === "children" ? React.Children.toArray(propValue) : [propValue];
    if (required && instances.length === 0) {
      return new Error(
        `${componentName} requires ${types.join("|")} ${propName}`
      );
    }
    for (const instance of instances) {
      ...
      if (
        instance &&
        (!instance.type || types.every((t) => instance.type.$$udType !== t))
      ) {
        return new Error(
          `${componentName} only accepts ${types.join("|")} ${propName}`
        );
      }
    }
  };
