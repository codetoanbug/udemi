...
export class OneTrustConsent {
  ...
  static allowsGoogleTagManager(urlSearchParams) {
    ...
    if (
      this.isUb() ||
      this.requiresCookieOpIn(oneTrust) ||
      this.isInCalifornia(oneTrust)
    ) {
  ...
  static toGtagEventData() {
    return {
      ad_storage: this.allows(OneTrustConsentTypes.ADVERTISING)
        ? "true"
        : "false",
      analytics_storage: this.allows(OneTrustConsentTypes.PERFORMANCE)
        ? "true"
        : "false",
      functionality_storage: this.allows(OneTrustConsentTypes.FUNCTIONAL)
        ? "true"
        : "false",
