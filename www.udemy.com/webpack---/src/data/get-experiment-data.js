...
export function resetExperimentData() {
  serverOrClient.global.UD.experiment = {};
}
...
export function getVariantValueFromUdRequest(
  experimentSet,
  variantName,
  defaultValue
) {
  ...
  if (
    extraUdExperiment === undefined ||
    extraUdExperiment[experimentSet] === undefined
  ) {
    return defaultValue;
  }
...
export function getNonRecordedExperimentIdsFromVariantKey(
  experimentSet,
  variantKey
) {
  const udExperiment = getExperimentData();
  ...
  const experimentIds = Object.keys(experimentsInSet).filter((id) => {
    return (
      variantKey in experimentsInSet[id].values &&
      experimentsInSet[id].state === "unrecorded"
    );
  });
