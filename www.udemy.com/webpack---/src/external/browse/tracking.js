...
function udLiteTrackDiscoveryImpression(
  ...
  trackDiscoveryImpression(item, backendSource, index, {
    relatedSourceId,
    relatedSourceType,
  });
}
...
const trackAutoCompleteResultClick = (position, autoCompleteResult) => {
  ...
  if (
    autoCompleteResult.type === "course" ||
    autoCompleteResult.type === "rvc"
  ) {
    setRefIDToLocalStorage(
      autoCompleteResult.id,
      autoCompleteResult.tracking_id
    );
  }
...
function trackAutoCompleteResultsImpression(trackingId, query) {
  ...
  const autoCompleteResultsImpressionEvent =
    new AutoCompleteResultImpressionEvent({
      resultTrackingId,
      query,
    });
...
function trackAutoCompleteClearHistoryClick(trackingId) {
  ...
  const autoCompleteClearHistoryClickEvent =
    new AutoCompleteClearHistoryClickEvent(resultTrackingId);
...
function trackCourseShareToChannelEvent(courseId, channel) {
  const courseShareToChannelEvent = new CourseShareToChannelEvent(
    courseId,
    channel
  );
