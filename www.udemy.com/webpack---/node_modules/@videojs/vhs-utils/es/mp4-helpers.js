...
DESCRIPTORS = [
  {
    ...
    parser: function parser(bytes) {
      ...
      if (desc.flags & 0x40) {
        ...
        desc.url = bytesToString(
          bytes.subarray(desc.size + 1, desc.size + 1 + len)
        );
        desc.size += len;
      } // ocr es id
...
export var buildFrameTable = function buildFrameTable(stbl, timescale) {
  ...
  var timeToSamples = parseSamples(
    findBox(stbl, ["stts"])[0],
    8,
    function (entry) {
      return {
  ...
  var samplesToChunks = parseSamples(
    findBox(stbl, ["stsc"])[0],
    12,
    function (entry) {
      return {
  ...
  var sampleSizes = parseSamples(
    (stsz && stsz.length && stsz.subarray(4)) || null
  );
  var frames = [];
  ...
  for (var chunkIndex = 0; chunkIndex < chunkOffsets.length; chunkIndex++) {
    ...
    for (var i = 0; i < samplesToChunks.length; i++) {
      ...
      var isThisOne =
        chunkIndex + 1 >= sampleToChunk.firstChunk &&
        (i + 1 >= samplesToChunks.length ||
          chunkIndex + 1 < samplesToChunks[i + 1].firstChunk);
    ...
    for (var _i = 0; _i < samplesInChunk; _i++) {
      ...
      for (var k = 0; k < timeToSamples.length; k++) {
        ...
        if (frames.length <= sampleCount) {
          // ms to ns
          var lastTimestamp = frames.length
...
export var addSampleDescription = function addSampleDescription(track, bytes) {
  ...
  if (codec === "avc1") {
    ...
    var decoderConfig =
      esDescriptor &&
      esDescriptor.descriptors.filter(function (_ref) {
        var tag = _ref.tag;
        return tag === 0x04;
      })[0];
...
export var parseTracks = function parseTracks(bytes, frameTable) {
  ...
  traks.forEach(function (trak) {
    ...
    if (tkhd) {
      ...
      track.number =
        tkhdVersion === 0 ? view.getUint32(12) : view.getUint32(20);
    }
