...
var union = function union(lists, keyFunction) {
  return values(
    lists.reduce(function (acc, list) {
...
var urlTypeToSegment = function urlTypeToSegment(_ref) {
  ...
  if (range || indexRange) {
    ...
    var startRange = window.BigInt
      ? window.BigInt(ranges[0])
      : parseInt(ranges[0], 10);
    var endRange = window.BigInt
      ? window.BigInt(ranges[1])
      : parseInt(ranges[1], 10); // convert back to a number if less than MAX_SAFE_INTEGER

    if (
      startRange < Number.MAX_SAFE_INTEGER &&
      typeof startRange === "bigint"
    ) {
    ...
    if (typeof endRange === "bigint" || typeof startRange === "bigint") {
      length =
        window.BigInt(endRange) - window.BigInt(startRange) + window.BigInt(1);
    } else {
...
var byteRangeToString = function byteRangeToString(byterange) {
  ...
  if (
    typeof byterange.offset === "bigint" ||
    typeof byterange.length === "bigint"
  ) {
    endRange =
      window.BigInt(byterange.offset) +
      window.BigInt(byterange.length) -
      window.BigInt(1);
  } else {
...
var segmentRange = {
  ...
  dynamic: function dynamic(attributes) {
    var NOW = attributes.NOW,
      ...
      periodStart =
        _attributes$periodSta === void 0 ? 0 : _attributes$periodSta,
    ...
    return {
      start: Math.max(0, availableStart),
        ...
        typeof endNumber === "number"
          ? endNumber
          : Math.min(segmentCount, availableEnd),
    };
  },
};
/**
...
var parseByDuration = function parseByDuration(attributes) {
  ...
  if (type === "static") {
    ...
    var sectionDuration =
      typeof periodDuration === "number" ? periodDuration : sourceDuration; // final segment may be less than full segment duration
...
var addSidxSegmentsToPlaylist$1 = function addSidxSegmentsToPlaylist(
  playlist,
  sidx,
  baseUrl
) {
...
var getMediaGroupPlaylists = function getMediaGroupPlaylists(manifest) {
  ...
  forEachMediaGroup(
    ...
    function (properties, type, group, label) {
      mediaGroupPlaylists = mediaGroupPlaylists.concat(
...
var updateMediaSequenceForPlaylist = function updateMediaSequenceForPlaylist(
  _ref2
) {
...
var updateSequenceNumbers = function updateSequenceNumbers(_ref3) {
  ...
  newPlaylists.forEach(function (playlist) {
    ...
    var oldPlaylist = findPlaylistWithName(
      oldPlaylists,
    ...
    var oldMatchingSegmentIndex = findIndex(
    ...
    if (oldMatchingSegmentIndex === -1) {
      ...
      if (
        (!oldPlaylist.segments.length &&
          playlist.timeline > oldPlaylist.timeline) ||
        (oldPlaylist.segments.length &&
          playlist.timeline >
...
var positionManifestOnTimeline = function positionManifestOnTimeline(_ref5) {
  ...
  // one or the old one.
  var oldPlaylists = oldManifest.playlists.concat(
    getMediaGroupPlaylists(oldManifest)
  );
  var newPlaylists = newManifest.playlists.concat(
    getMediaGroupPlaylists(newManifest)
...
var addSidxSegmentsToPlaylist = function addSidxSegmentsToPlaylist(
  playlist,
  sidxMapping
) {
...
var addSidxSegmentsToPlaylists = function addSidxSegmentsToPlaylists(
  playlists,
  sidxMapping
) {
...
var organizeAudioPlaylists = function organizeAudioPlaylists(
  ...
  var formattedPlaylists = playlists.reduce(function (a, playlist) {
    ...
    var formatted = addSidxSegmentsToPlaylist(
...
var organizeVttPlaylists = function organizeVttPlaylists(
  ...
  return playlists.reduce(function (a, playlist) {
    ...
    a[label].playlists.push(
      addSidxSegmentsToPlaylist(formatVttPlaylist(playlist), sidxMapping)
    );
    return a;
  }, {});
};

var organizeCaptionServices = function organizeCaptionServices(
  captionServices
) {
...
var videoOnly = function videoOnly(_ref4) {
  ...
  return (
    ...
    attributes.mimeType === "video/webm" ||
...
var audioOnly = function audioOnly(_ref5) {
  ...
  return (
    ...
    attributes.mimeType === "audio/webm" ||
...
var addMediaSequenceValues = function addMediaSequenceValues(
  playlists,
  timelineStarts
) {
...
var flattenMediaGroupPlaylists = function flattenMediaGroupPlaylists(
  mediaGroupObject
) {
  if (!mediaGroupObject) {
    return [];
...
var toM3u8 = function toM3u8(_ref8) {
  ...
  var _dashPlaylists$0$attr = dashPlaylists[0].attributes,
    ...
    suggestedPresentationDelay =
      _dashPlaylists$0$attr.suggestedPresentationDelay,
  ...
  var organizedAudioGroup = audioPlaylists.length
    ? organizeAudioPlaylists(audioPlaylists, sidxMapping, isAudioOnly)
    : null;
  var organizedVttGroup = vttPlaylists.length
    ? organizeVttPlaylists(vttPlaylists, sidxMapping)
    : null;
  var formattedPlaylists = videoPlaylists.concat(
  ...
  if (captions.length) {
    manifest.mediaGroups["CLOSED-CAPTIONS"].cc =
      organizeCaptionServices(captions);
  }
...
var segmentsFromTemplate = function segmentsFromTemplate(
  ...
  return segments.map(function (segment) {
    ...
    var presentationTime = // Even if the @t attribute is not specified for the segment, segment.time is
      ...
      attributes.periodStart +
      (segment.time - presentationTimeOffset) / timescale;
    var map = {
      uri: uri,
...
var parseCaptionServiceMetadata = function parseCaptionServiceMetadata(
  service
) {
  // 608 captions
  if (service.schemeIdUri === "urn:scte:dash:cc:cea-608:2015") {
    var values =
      typeof service.value !== "string" ? [] : service.value.split(";");
...
var toRepresentations = function toRepresentations(
  ...
  return function (adaptationSet) {
    ...
    var captionServices = parseCaptionServiceMetadata(
      parseAttributes(accessibility)
    );
...
var toAdaptationSets = function toAdaptationSets(mpdAttributes, mpdBaseUrls) {
  return function (period, index) {
    var periodBaseUrls = buildBaseUrls(
      mpdBaseUrls,
      findChildren(period.node, "BaseURL")
    );
...
var inheritAttributes = function inheritAttributes(mpd, options) {
  ...
  return {
    ...
    representationInfo: flatten(
      periods.map(toAdaptationSets(mpdAttributes, mpdBaseUrls))
    ),
  };
};
...
var parse = function parse(manifestString, options) {
  ...
  var parsedManifestInfo = inheritAttributes(
    stringToMpdXml(manifestString),
    options
  );
