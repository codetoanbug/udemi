 ...
 */
export const udGtag = function (command, commandParameters) {
  let additionalData =
  ...
  if (command === "event") {
    ...
    const eventName = commandParameters; // readability
...
export function createGlobalSafeAPI() {
  ...
  if (
    Array.isArray(udGtagInvocations) &&
    udGtagInvocations &&
    udGtagInvocations.length > 0
  ) {
 ...
 */
export function loadGtag(pageConfig, scriptLoadedCallback) {
  let scriptLoader =
    arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : $script;
  let overrideUniqueTrackingId =
  ...
  saveEventData({
    ...
    external_id: `${sha256(`${visitorUuid}`)}`,
  });
  ...
  return Promise.all(promises()).then(() => {
    ...
    if (
      consentData.ad_storage !== "true" ||
      consentData.personalization_storage !== "true"
    ) {
      return;
    }
    udGtag(
...
//# sourceMappingURL=ud-server-side-gtag.js.map
