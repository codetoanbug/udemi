...
export function useUpdateUserLocale() {
  ...
  const [queuedLocaleUpdate, setQueuedLocaleUpdate] = React.useState();
  React.useEffect(() => {
    if (!data.isGlobalMeContextLoading && queuedLocaleUpdate) {
      if (data.me.is_authenticated) {
        udApi
          .patch("users/me/", {
            locale: queuedLocaleUpdate,
          })
          .catch(() => {
            // Swallow error
          });
      ...
      Cookies.set("ud_locale", queuedLocaleUpdate, {
        expires: 1,
      });
      Cookies.set("ud_cache_language", queuedLocaleUpdate.slice(0, 2), {
        expires: 1,
      });
...
//# sourceMappingURL=use-update-user-locale.js.map
