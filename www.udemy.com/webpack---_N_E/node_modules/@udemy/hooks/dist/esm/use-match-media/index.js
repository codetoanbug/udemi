...
export const normalizeMediaQuery = (mediaQueryStringOrKey) => {
  if (breakpointKeys.includes(mediaQueryStringOrKey)) {
    const maxOrMinWidth = mediaQueryStringOrKey.endsWith("-max")
      ? "max-width"
      : "min-width";
    const tokenKey = `${breakpointPrefix}${mediaQueryStringOrKey}`;
    const width = tokens[tokenKey];
    return `(${maxOrMinWidth}: ${width})`;
  }
  return mediaQueryStringOrKey;
};
 ...
 */
export const useMatchMedia = (mediaQueryStringOrKey) => {
  const mediaQueryString = normalizeMediaQuery(mediaQueryStringOrKey);
  const mediaQueryList = useMemo(() => {
    var _window$matchMedia, _window;
    return (
      typeof window !== "undefined" &&
      ((_window$matchMedia = (_window = window).matchMedia) === null ||
      _window$matchMedia === void 0
        ? void 0
        : _window$matchMedia.call(_window, mediaQueryString))
    );
  }, [mediaQueryString]);
 ...
 */
export const withMatchMedia = (mediaQueryStringMap) => {
  return (WrappedComponent) => (props) => {
    const mqlMap = Object.entries(mediaQueryStringMap).reduce((acc, _ref) => {
      let [key, mediaQueryString] = _ref;
      return {
        ...acc,
