...
const onKeys = function () {
  ...
  return (fn) => (event) => {
    ...
    if (
      _includesInstanceProperty((_context = ["keydown", "keypress"])).call(
        _context,
        event.type
      ) &&
      _includesInstanceProperty(keys).call(keys, event.key)
    ) {
      fn(event);
    }
  };
...
export const uniqueIdNamespace = () => {
  ...
  const setUniqueIdNamespace = function () {
    let value =
