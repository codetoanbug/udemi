...
export const fetchPopularTopicsForSubcategory = async function (
  subcategoryId,
  locale
) {
  let stat_size =
  ...
  */
  ...
  const popularTopicsOld = response.data;
  const popularTopics = {
    id: subcategoryId,
    stats: popularTopicsOld.results.map((popularTopicOld) => {
      return {
...
export const fetchMostPopularTopicsForSubcategories = async function (locale) {
  let isUFB =
  ...
  const popularTopics = popularTopicsPromises
    ...
    .reduce((acc, _ref) => {
      ...
      return {
        ...acc,
        [value.id]: value.stats,
      };
    }, {});
  return popularTopics;
};
//# sourceMappingURL=fetch-popular-topics.js.map
