 ...
 * @returns {null|number}
 */
export const convertToKB = (bytes) => {
...
export const getIsLowEndDevice = () => {
  ...
  if (getHardwareConcurrency() && getHardwareConcurrency() <= 4) {
    return true;
  }
 ...
 */
export const getIsLowEndExperience = (effectiveConnectionType, saveData) => {
 ...
 * @returns None
 */
export function getDeviceInformation(reportFn) {
  let ect = null;
  let saveData = null;
  ...
  reportFn("isLowEndExperience", getIsLowEndExperience(ect, saveData));
  if (
    window.navigator.storage &&
    typeof window.navigator.storage.estimate === "function"
  ) {
    window.navigator.storage.estimate().then((storageInfo) => {
      reportFn("storageQuota", convertToKB(storageInfo.quota));
      reportFn("storageUsage", convertToKB(storageInfo.usage));
    });
  }
  reportFn("url", window.location.href);
}
//# sourceMappingURL=get-device-information.js.map
