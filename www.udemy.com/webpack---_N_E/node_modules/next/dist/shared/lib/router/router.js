...
var _resolveRewrites = _interop_require_default(
  require("./utils/resolve-rewrites")
);
    ...
    if (rewriteTarget.startsWith("/")) {
      ...
      return Promise.all([
        ...
        if (process.env.__NEXT_HAS_REWRITES) {
          const result = (0, _resolveRewrites).default(
            as,
            pages,
            rewrites,
  ...
  const getData = (params) => {
    return fetchRetry(dataHref, isServerRender ? 3 : 1, {
      headers: isPrefetch
        ...
        : {},
      method:
        (ref1 = params == null ? void 0 : params.method) != null ? ref1 : "GET",
    })
      .then((response) => {
        if (
          response.ok &&
        ...
        return response.text().then((text) => {
          if (!response.ok) {
            ...
            if (!hasMiddleware && response.status === 404) {
              var ref;
              if (
                (ref = tryToParseAsJSON(text)) == null ? void 0 : ref.notFound
              ) {
                return {
                  dataHref,
                  json: {
          ...
          _async_to_generator(function* () {
            ...
            return {
              ...
              props: yield _this.getInitialProps(
                routeInfo.Component, // we provide AppTree later so this needs to be `any`
                {
                  pathname,
                  query,
                  asPath: as,
   ...
   */ prefetch(url, asPath = url, options = {}) {
    ...
    return _async_to_generator(function* () {
      ...
      if (process.env.__NEXT_HAS_REWRITES && asPath.startsWith("/")) {
        let rewrites;
        ({ __rewrites: rewrites } = yield (0,
        _routeLoader).getClientBuildManifest());
        const rewritesResult = (0, _resolveRewrites).default(
          (0, _addBasePath).addBasePath(
            (0, _addLocale).addLocale(asPath, _this.locale),
            true
          ),
          pages,
          rewrites,
        ...
        resolvedAs = (0, _removeLocale).removeLocale(
          (0, _removeBasePath).removeBasePath(rewritesResult.asPath),
          _this.locale
        );
        if (rewritesResult.matchedPage && rewritesResult.resolvedHref) {
          // if this directly matches a page we need to update the href to
