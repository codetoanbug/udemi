...
import { getUniqueId } from "udemy-django-static/js/utils/a11y";
import applyValidators, {
  validateExcludeExternalURL,
} from "udemy-django-static/js/utils/form-validators";
...
export default class Question {
  ...
  getNextAnswerOrCreate = (key) => {
    const indexCurrent = this.answerList.findIndex(
  ...
  validateAnswer(answer) {
    ...
    const validationOutcome = applyValidators(
      [validateExcludeExternalURL],
      item
    );
    if (validationOutcome === "error") {
      ...
      this.maxCharCount > 0 &&
      answer.value.length > this.maxCharCount
    ) {
  ...
  get nonEmptyAnswerCount() {
    return this.answerList.filter(
      (x) => x.validationState !== "error" && x.value !== ""
    ).length;
  }
