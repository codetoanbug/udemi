...
const store = storeEngine.createStore(
  [storeLocalStorage, storeMemoryStorage],
  []
);
 ...
 */
export default function expiringLocalStorageFactory(namespace, name, date) {
  /*
  ...
  state.expirations.observe(() => {
    ...
    const updatedExpirations = Object.assign(
      expirations,
      toJS(state.expirations)
    );
  ...
  action(() => {
    ...
    state.expirations.forEach((dateString, key) => {
      if (new Date(dateString) < now) {
        store.remove(key);
        const expirations = store.get(
          "expiringLocalStorageFactory.expirations"
        );
  ...
  return {
     ...
     */
    updateExpiration: action((newExpirationDate) => {
