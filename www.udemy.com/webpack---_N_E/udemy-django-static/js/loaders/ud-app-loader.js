 ...
 * For UD pages, this function imports from app-name/app.js.
 * For UDLite pages, this function imports from app-name/udlite-app.js.
 *
...
export default function udAppLoader(container = document, callback = noop) {
  ...
  function load(elem) {
    ...
    if (
      elem.classList.contains("ud-app-loading") ||
      elem.classList.contains("ud-app-loaded")
    ) {
    ...
    return _importApp(moduleId)
      ...
      .catch((error) => {
        udSentry.captureException(error);
      });
  }
