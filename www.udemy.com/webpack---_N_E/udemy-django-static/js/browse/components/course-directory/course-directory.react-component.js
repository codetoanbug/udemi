...
import { CollectionTypeSwitchEvent } from "udemy-django-static/js/browse/events";
import { CollectionTypes } from "udemy-django-static/js/browse/lib/constants";
...
import styles from "./course-directory.module.less";
...
const CourseDirectory = inject("isConsumerSubsSubscriber")(
  ({
    ...
    const showSwitchToPPCollectionLink =
      isConsumerSubsSubscriber &&
      searchParams.get("subs_filter_type") === "purchasable_only";
    ...
    const handleSubsOnlyFilter = (e) => {
      ...
      if (searchParams.get("homeOnSwitch")) {
        ...
        switchCollectionType("subs_only", router, searchParams);
      }
    };
...
CourseDirectory.propTypes = {
  ...
  pagination: PropTypes.shape({ total_page: PropTypes.number.isRequired })
    .isRequired,
...
export function switchCollectionType(
  toType,
  router,
  currentParams,
  homeOnSwitch = false
  ...
  router.push(
    { pathname: router.asPath.split("?")[0], query: queryParams.toString() },
    undefined,
    {
      scroll: false,
      shallow: true,
...
const CollectionTypeSwitchLink = ({ handleClick, query, count }) => {
  ...
  const handlePurchaseOnly = (e) => {
    e.preventDefault();
    switchCollectionType("purchasable_only", router, searchParams);
    onCollectionTypeChange(
      query,
      count,
      CollectionTypes.CONSUMERSUBSCRIPTION,
  ...
  return (
    <div
      className={styles["pp-tooltip-wrapper"]}
