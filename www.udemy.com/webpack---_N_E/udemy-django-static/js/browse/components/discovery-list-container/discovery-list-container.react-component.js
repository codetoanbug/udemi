...
import CourseDirectory from "udemy-django-static/js/browse/components/course-directory/course-directory.react-component";
...
export class InternalDiscoveryListContainer extends React.Component {
  ...
  constructor(props) {
    super(props);
    const { router, pageObjectId, pageType, unit } = this.props;
  ...
  render() {
    if (this.store.unit === null && this.store.loading) {
      return (
        <Loader
          size="medium"
          block={true}
          className={styles["loader-spacing"]}
        />
      );
    }
    ...
    const { onPageChange, filterOrder, query, courseDirectoryProps, udData } =
      this.props;
    const { Config } = udData;
