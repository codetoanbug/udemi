...
import { switchCollectionType } from "udemy-django-static/js/browse/components/course-directory/course-directory.react-component";
import { DiscoveryItemImpressionEvent } from "udemy-django-static/js/browse/events";
import DiscoveryAPI from "udemy-django-static/js/browse/lib/backends/discovery-api";
...
export default class DiscoveryListContainerStore {
  constructor(
    { unit = null, pageType, pageObjectId, router, gettext },
    globalOverrides = {}
  ) {
  ...
  backendSource =
    DiscoveryItemImpressionEvent.backendSourceOptions.DISCOVERY_ALL_COURSES;

  @action
  async fetchUnit(options = {}) {
    this.options = options;
    const { selectedFilters, selectedTopicIds, sortBy, ...passthroughOptions } =
      options;
    const topicFilter =
