 ...
 */
export const combineAndSortCourseArrays = (
  purchasedCourses,
  consumerSubscriptionCourses
) => {
  let courses = [...purchasedCourses, ...consumerSubscriptionCourses];
  const hasAnyActivationCard = courses.find(
    (item) => item.title === "activation-card"
  );
  ...
  courses.sort(
    (a, b) =>
      new Date(b.last_accessed_time).getTime() -
      new Date(a.last_accessed_time).getTime()
  );
