...
import { AutoPlayCarousel } from "@udemy/react-structure-components";
...
import StartLearningLabel from "udemy-django-static/js/browse/components/course-card/start-learning-label.react-component";
...
import styles from "./single-course-unit.module.less";
  ...
  return (
    ...
    >
      ...
      {numPublishedLectures && (
        <span>
          {ninterpolate("%s lecture", "%s lectures", numPublishedLectures)}
        </span>
      )}
...
export class InternalCourseCard extends Component {
  ...
  trackClick(event) {
    const courseId = parseInt(
      event.currentTarget.getAttribute("data-course-id"),
      10
    );
    const courseTrackingId = event.currentTarget.getAttribute(
  ...
  render() {
    ...
    const CodingExercisesBadgeComponent =
      ...
      courseBadgeFromType(COURSE_BADGE_CODING_EXERCISES);

    const courseWrapperClass = classNames(
      styles.container,
      styles["course-wrapper"],
      {
        [styles["course-wrapper__multi"]]:
...
export default class SingleCourseUnit extends Component {
  ...
  render() {
    ...
    const CarouselComponent = hasPrimaryCoarsePointer
      ? Carousel
      : AutoPlayCarousel;
