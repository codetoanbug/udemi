...
import UnitProcessor from "udemy-django-static/js/browse/lib/unit-processor";
...
export default class DiscoveryUnitsContainerStore {
  ...
  fetchUnits = async (options) => {
    try {
      ...
      const response = await this.discoveryAPI.loadUnits(
        this.pageType,
        requestOptions
      );
  ...
  processPreloadedUnits() {
    ...
    this.units = UnitProcessor.processUnits(
      requestOptions,
      this.units,
      this.pageType
    );
