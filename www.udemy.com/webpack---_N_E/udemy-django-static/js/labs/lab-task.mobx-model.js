...
import { AutomatedReviewTest } from "udemy-django-static/js/lab-manage/tasks/automated-review-tests/automated-review-test.mobx-model";
import { AutomatedReviewTestRun } from "udemy-django-static/js/lab-taking/tasks/structured/automated-lab-review/automated-review-test-run.mobx-model";
import {
  RESOURCE_TYPE_ASSET,
...
export default class LabTask extends EditableApiModel {
    ...
    return {
      ...
      resources: {
        ...
        map: (resources) => {
          return resources.map(
            (resource) => new LabTaskResource(resource, this)
          );
        },
      },
      ...
      automatedReviewTests: {
        ...
        map: (automatedReviewTests) => {
          return automatedReviewTests.map(
            (test) => new AutomatedReviewTest(test, this)
          );
        },
      },
  ...
  get isCompleted() {
    return (
      !!this.lab.currentMode && this.completionData.has(this.lab.currentMode)
    );
  }
  ...
  get howTo() {
    return this.resources.filter(
      (resource) => resource.type === RESOURCE_TYPE_HOW_TO
    );
  }
  ...
  get documentation() {
    return this.resources.filter(
      (resource) => resource.type === RESOURCE_TYPE_DOCUMENT
    );
  }
  ...
  get assets() {
    return this.resources.filter(
      (resource) => resource.type === RESOURCE_TYPE_ASSET
    );
  }
  ...
  get azureResources() {
    return this.resources.filter(
      (resource) => resource.type === RESOURCE_TYPE_AZURE_RESOURCE
    );
  }
  ...
  get isSavingInProgress() {
    return (
      this.isSaving || this.resources.some((resource) => resource.isSaving)
    );
  }
  ...
  get isSavingError() {
    return (
      this.apiError || this.resources.some((resource) => resource.apiError)
    );
  }
  ...
  async createSolutionCode(assetId) {
    await this._createResource({
      type: RESOURCE_TYPE_SOLUTION_SOURCE_CODE,
      asset: assetId,
    });
  }
  ...
  async updateAutomatedReviewTestResource(automatedTestId, assetId, uiMode) {
    this._setSaving(true);
