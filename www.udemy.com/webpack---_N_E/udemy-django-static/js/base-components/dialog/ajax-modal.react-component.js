...
import AjaxContentStore from "udemy-django-static/js/base-components/dialog/ajax-content.mobx-store";
...
import styles from "./ajax-modal.module.less";

@observer
export default class AjaxModal extends Component {
  static propTypes = {
...
- `<div class="run-command" />` is parsed and evaluated.
  ...
  onBootstrapContents = (contentNode) => {
  ...
  applyInterceptor = () => {
    if (this.props.shouldInterceptContent) {
      ...
      const links = this.contentNode.querySelectorAll(
      ...
      const forms = this.contentNode.querySelectorAll(
  ...
  onSubmitForm = async (event) => {
    ...
    try {
      ...
      this.store.setContent(gettext("An unknown error occurred"), true);
    }
  };
  ...
  render() {
    const { skipCache, url, labelledById, className, ...modalProps } =
      this.props;
    ...
    return (
      <Modal
        ...
        className={classNames(className, {
          [styles["ajax-error"]]: this.store.hasErrorContent,
        })}
