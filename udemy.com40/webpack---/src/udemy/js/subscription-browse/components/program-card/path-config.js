...
export default function pathConfig(path = 'default') {
    return pathMap[path] || pathMap.default;
}

export function pathBarStyle(path) {
    const color = pathConfig(path).color;
    return {borderLeft: `${pxToRem(8)}rem solid ${tokens[color]}`};
}
