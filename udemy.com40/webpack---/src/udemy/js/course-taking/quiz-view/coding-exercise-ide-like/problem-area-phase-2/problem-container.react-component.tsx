...
const ProblemContainerComponent: React.FC<{courseTakingStore: CourseTakingStore}> = ({
    ...
    useEffect(
        (() => {
            codingExerciseStore.loadUnlockedPopoversDismissed();
        }) as VoidFunction,
        [],
    );
    ...
    useEffect(() => {
        codingExerciseStore.solutionUnlockedPopoverOpen &&
            codingExerciseStore.hideHintsUnlockedPopover();
    }, [codingExerciseStore, codingExerciseStore.solutionUnlockedPopoverOpen]);
    ...
    const renderTabButton = (
        unlocked: boolean,
        isUnlockedPopoverOpen: boolean,
        isLockedPopoverOpen: boolean,
        isPopoverCouldBeShown: boolean,
        showLockedPopover: React.MouseEventHandler<HTMLDivElement> | undefined,
        hideLockedPopover: React.MouseEventHandler<HTMLDivElement> | undefined,
        hideUnlockedPopover: React.MouseEventHandler<HTMLElement> | undefined,
        lockedText: string,
        unlockedText: string,
    ) => (tabButton: React.ReactElement) => {
        const button = unlocked
            ? tabButton
            : React.cloneElement(tabButton, {'aria-disabled': true, onClick: showLockedPopover});
        return (
            <Popover
                placement="bottom"
                detachFromTarget={true}
                isOpen={(isUnlockedPopoverOpen || isLockedPopoverOpen) && isPopoverCouldBeShown}
                trigger={
                    <div styleName="tab-button-container">
                        {button}
                        {!unlocked && <LockIcon color="subdued" label={false} size="xsmall" />}
                    </div>
                }
                withArrow={true}
                withPadding={false}
            >
                <div styleName="popover-content">
                    <div className="ud-text-md" data-purpose="popover-text">
                    ...
                    <div>
                        <Button
                            udStyle="secondary"
                            size="medium"
                            onClick={unlocked ? hideUnlockedPopover : hideLockedPopover}
                        >
                            {gettext('Dismiss')}
                        </Button>
                    </div>
                            ...
                            <div styleName="tab-container-inner">
                                <Tabs
                                    ...
                                    onSelect={(id) => {
                                        ...
                                        codingExerciseStore.trackEvent(
                                            id as string,
                                            eventUITypeValue,
                                        );
                                    }}
                                >
                                    ...
                                    {prompt.hints?.[0]?.description && (
                                        <Tabs.Tab
                                            ...
                                            renderTabButton={renderTabButton(
                                                codingExerciseStore.isHintsTabUnlocked,
                                                codingExerciseStore.hintsUnlockedPopoverOpen,
                                                codingExerciseStore.hintsLockedPopoverOpen,
                                                codingExerciseStore.isPopoverCouldBeShown,
                                                codingExerciseStore.showHintsLockedPopover,
                                                codingExerciseStore.hideHintsLockedPopover,
                                                codingExerciseStore.hideHintsUnlockedPopover,
                                                gettext(
                                                    'Hints will be available after your second failed attempt or/and when you pass this exercise.',
                                                ),
                                                gettext(
                                    ...
                                    {prompt.solution && (
                                        <Tabs.Tab
                                            ...
                                            id="solution"
                                            renderTabButton={renderTabButton(
                                                codingExerciseStore.isSolutionTabUnlocked,
                                                codingExerciseStore.solutionUnlockedPopoverOpen,
                                                codingExerciseStore.solutionLockedPopoverOpen,
                                                codingExerciseStore.isPopoverCouldBeShown,
                                                codingExerciseStore.showSolutionLockedPopover,
                                                codingExerciseStore.hideSolutionLockedPopover,
                                                codingExerciseStore.hideSolutionUnlockedPopover,
                                                gettext(
                                                    'Solution explanation will be available after your third failed attempt or/and when you pass this exercise.',
                                                ),
                                                gettext(
                                                    "Check your instructor's solution to learn how to solve this exercise.",
                                                ),
                                            )}
                                        />
                                    )}
                    ...
                    <div styleName="tab-content" data-purpose="tab-content">
                        {activeTab === 'instructions' && (
                            <Instructions
                                ...
                                lectures={assessment.related_lectures}
                                store={codingExerciseStore}
                                learningObjective={prompt.learning_objective}
                            />
                        )}
                        {activeTab === 'hints' && (
