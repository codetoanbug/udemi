...
export const supportKeySystem = memoize(async (keySystem, requiredConfigs) => {
 ...
 */
export const patchHttpStreamingSourceHandlers = async function (videojs) {
...
const isWidevineDrmError = function (error) {
    ...
    return error && error.response && [403].includes(error.response.status);
};
...
export const buildLicenseServerURL = function (drmType, authToken) {
    return DRM_LICENSE_SERVER_URL_TEMPLATE.replace('{drm_type}', drmType)
        .replace('{auth_token}', authToken)
        .replace('{base_url}', baseURL);
};

export const getContentIdForFairPlay = function (emeOptions, initData) {
...
export const getLicenseForWidevine = function (authToken) {
    const requestURL = buildLicenseServerURL('widevine', authToken);

    return async function (emeOptions, keyMessage, callback) {
        try {
...
function parsePlayReadyKeyMessage(keyMessage) {
...
export const getLicenseForPlayReady = function (authToken) {
...
export const getLicenseForFairPlay = function (authToken) {
    const requestURL = buildLicenseServerURL('fairplay', authToken);

    return async function (emeOptions, contentId, keyMessage, callback) {
...
export const buildKeySystemsConfig = function (authToken) {
    return {
        [WIDEVINE_KEY_SYSTEM]: {
...
export const trackPlayerDRMPerformance = function (player) {
    ...
    playerTech.on('keystatuschange', (event) => {
