 ...
 */
export function parseDateString(dateString, locale) {
    if (!locale) {
        locale = getRequestData().locale.replace('_', '-') || 'en-US';
    }
...
const COMPARE_UNIT = Object.freeze({
    DAY: 'day',
    MONTH: 'month',
});
 ...
 */
export function compare(date1, date2, unit = COMPARE_UNIT.DAY) {
...
const INCREMENT_UNIT = Object.freeze({
    DAY: 'day',
    MONTH: 'month',
    YEAR: 'year',
});

export function increment(date, amount, unit) {
    const newDate = new Date(date);
